...
parser.add_argument('--data_dir', type=str, default="datasets/700_images")
...
def main(args):
    ...
    while pbar.n < (50000 - args.train_step):
        for x in train_loader:
            distort_x = distort_images3(x)  # 扭曲图片
            distort_x = distort_x.to(gpu_cn)
            #rotate_x = process_image(x)  # 旋转图片
            #rotate_x = rotate_x.to(gpu_cn)
            x = x.to(gpu_cn)
